...
<div class="mb-3">
    <x-form.label for="credentials[group_url]" label="{{ localize('Group Url') }}" isRequired=true />
    <x-form.input id="credentials[group_url]" name="credentials[group_url]" type="text" placeholder="{{ localize('Group Url') }}" value="{{$platform->credentials ? json_decode($platform->credentials)->group_url ?? ''  : ''}}" showDiv=false  required/>
</div>
<div class="mb-3">
    <x-form.label for="credentials[callback_url]" label="{{ localize('Callback Url') }}" />
    <x-form.input id="credentials[callback_url]" name="credentials[callback_url]" type="text" placeholder="{{ localize('Callback Url') }}" value="{{ route('social.accounts.callback', ['platform' => 'facebook']) }}" showDiv=false disabled/>
</div>