<script>
    ...
    function getDataList() {
        ...
        callParams.url      = "{{ route('admin.prompts.index') }}" + (gFilterObj ? '?' + $.param(gFilterObj) : '');
        callParams.data     = '';
    ...
    $('body').on('click', '#addFrmOffCanvas', function(){
        $('#addFrm .offcanvas-title').text("{{ localize('Add Prompt') }}");
        resetFormErrors('form#addFrm');
        resetForm('form#addFrm');
        showElement('.password_wrapper');
        $('form#addFrm').attr('action', "{{ route('admin.prompts.store') }}");
    ...
    $("#addFrm").submit(function(e) {
        ...
        ajaxCall(callParams, function (result) {
            console.log("Server Response ", result,"Message : ", result.message);
    ...
    $(document).on('click', '.editIcon', function(){
        ...
        $('#addFrm .offcanvas-title').text("{{ localize('Update Prompt') }}");
        ...
        ajaxCall(callParams, function (result) {
            ...
            if(result.data) {
                ...
                $('#addFrm #id').val(prompt.id);
                $('#addFrm #name').val(prompt.name);
                $('#addFrm #prompt').val(prompt.description);
                $('#addFrm #is_active').val(prompt.is_active).change();
                $('#addFrm #prompt_group_id').val(prompt.prompt_group_id).change();
            }
        },
        function (err, type, httpStatus) {
