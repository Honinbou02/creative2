...
@section('title')
    {{ localize('Prompt Groups Lists') }} {{ getSetting('tab_separator') }} {{ getSetting('system_title') }}
@endsection
@section("pagetitle", localize('Prompt Group'))

@section('breadcrumb') @php
    $breadcrumbItems = [
        ["href"  => null, "title" => "Prompt Groups"]
    ]; @endphp
...
@section('pageTitleButtons')
    <div class="col-auto">
        <x-form.button type="button"
            ...
            <i data-feather="plus"></i>{{ localize("Add Prompt Group") }}
        </x-form.button>
    </div>
@endsection
...
@section('content')
    ...
    <!-- Add Prompt Group start-->
    @include('backend.admin.prompt-groups.sidebar_add_prompt_group')
    <!-- Add Prompt Group -->

@endsection
@section('js')
    @include('backend.admin.prompt-groups.js')
@endsection
