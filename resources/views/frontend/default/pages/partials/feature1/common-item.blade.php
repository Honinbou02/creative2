@php
    ...
    $short     = "feature_document_{$itemNo}_short_description";
...
@if(!empty($headingTitle))
    <div class="col-md-6 col-lg-4">
        <div class="p-7 rounded-2 wt_card_style d-flex flex-column justify-content-between gap-3 h-100 wow animate__animated animate__fadeInUp animate__delay-0_2s">
            <div class="px-5 py-8 rounded-2 text-center wt_card_style_2">
                <h6 class="fs-20 mb-2"> {{ $headingTitle }}</h6>
                <p class="fs-14 mb-6">{{ systemSettingsLocalization($short) }}</p>
                <a href="{{ getSetting($btnLink) ?? "#" }}" class="btn btn-outline-light rounded-pill border-0 fw-normal d-inlne-flex align-items-center gap-2 text-body">
                    ...
                    {{ systemSettingsLocalization($btnText) }}
                </a>
            </div>
            <div class="text-center">
                <img src="{{avatarImage(getSetting("feature_document_{$itemNo}_image"))}}"
                     alt="Image"
                     class="img-fluid"
                />
            </div>
