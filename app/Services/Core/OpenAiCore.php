...
use Exception;
use App\Services\Core\OpenAiCoreUrl;
...
class OpenAiCore
{
    ...
    {
        $url = OpenAiCoreUrl::fineTuneModel();
        $this->baseUrl($url);
    ...
    {
        $model = "/$model";
        $url = OpenAiCoreUrl::fineTuneModel().$model;
        $this->baseUrl($url);
    ...
    public function complete($opts)
    {
        ...
        $url = OpenAiCoreUrl::completionURL($engine);
        unset($opts['engine']);
        $this->baseUrl($url);
    ...
    public function completion($opts, $stream = null)
    {
        ...
        $url = OpenAiCoreUrl::completionsURL();
        $this->baseUrl($url);
    ...
    {
        $url = OpenAiCoreUrl::imageUrl()."/generations";
        $this->baseUrl($url);
    ...
    {
        $url = OpenAiCoreUrl::imageUrl()."/edits";
        $this->baseUrl($url);
    ...
    {
        $url = OpenAiCoreUrl::imageUrl()."/variations";
        $this->baseUrl($url);
    ...
    {
        $engine = $opts['engine'] ?? $this->engine;
        $url = OpenAiCoreUrl::searchURL($engine);
        unset($opts['engine']);
        $this->baseUrl($url);
    ...
    {
        $url = OpenAiCoreUrl::classificationsUrl();
        $this->baseUrl($url);
    ...
    {
        $url = OpenAiCoreUrl::moderationUrl();
        $this->baseUrl($url);
    ...
    {
        $url = OpenAiCoreUrl::transcriptionsUrl();
        $this->baseUrl($url);
    ...
    {
        $url = OpenAiCoreUrl::translationsUrl();
        $this->baseUrl($url);
    ...
    {
        $file_id = "/$file_id";
        $url = OpenAiCoreUrl::filesUrl().$file_id;
        $this->baseUrl($url);
    ...
    public function retrieveFineTune($fine_tune_id)
    {
        ...
        $url = OpenAiCoreUrl::fineTuneUrl().$fine_tune_id;
        $this->baseUrl($url);
    ...
    {
        $fine_tune_id = "/$fine_tune_id";
        $url = OpenAiCoreUrl::fineTuneModel().$fine_tune_id;
        $this->baseUrl($url);
    ...
    {
        $url = OpenAiCoreUrl::engineUrl($engine);
        $this->baseUrl($url);
    ...
    public function createAssistant($data)
    {
        ...
        $url = OpenAiCoreUrl::assistantsUrl();
        $this->baseUrl($url);
    ...
    {
        $this->headers[] = 'OpenAI-Beta: assistants=v1';
        $url = OpenAiCoreUrl::assistantsUrl() . '/' . $assistantId;
        $this->baseUrl($url);
    ...
    {
        $this->headers[] = 'OpenAI-Beta: assistants=v1';
        $url = OpenAiCoreUrl::assistantsUrl() . '/' . $assistantId . '/files';
        $this->baseUrl($url);
    ...
    {
        $this->headers[] = 'OpenAI-Beta: assistants=v1';
        $url = OpenAiCoreUrl::assistantsUrl() . '/' . $assistantId . '/files/' . $fileId;
        $this->baseUrl($url);
    ...
    {
        $this->headers[] = 'OpenAI-Beta: assistants=v1';
        $url = OpenAiCoreUrl::threadsUrl() . '/' . $threadId;
        $this->baseUrl($url);
    ...
    {
        $this->headers[] = 'OpenAI-Beta: assistants=v1';
        $url = OpenAiCoreUrl::threadsUrl() . '/' . $threadId . '/messages';
        $this->baseUrl($url);
    ...
    {
        $this->headers[] = 'OpenAI-Beta: assistants=v1';
        $url = OpenAiCoreUrl::threadsUrl() . '/' . $threadId . '/messages/' . $messageId;
        $this->baseUrl($url);
    ...
    {
        $this->headers[] = 'OpenAI-Beta: assistants=v1';
        $url = OpenAiCoreUrl::threadsUrl() . '/' . $threadId . '/messages/' . $messageId . '/files/' . $fileId;
        $this->baseUrl($url);
    ...
    {
        $this->headers[] = 'OpenAI-Beta: assistants=v1';
        $url = OpenAiCoreUrl::threadsUrl() . '/' . $threadId . '/messages/' . $messageId . '/files';
        if (count($query) > 0) {
            $url .= '?' . http_build_query($query);
        }
        $this->baseUrl($url);
    ...
    {
        $this->headers[] = 'OpenAI-Beta: assistants=v1';
        $url = OpenAiCoreUrl::threadsUrl() . '/' . $threadId . '/runs';
        $this->baseUrl($url);
    ...
    {
        $this->headers[] = 'OpenAI-Beta: assistants=v1';
        $url = OpenAiCoreUrl::threadsUrl() . '/' . $threadId . '/runs/' . $runId;
        $this->baseUrl($url);
    ...
    {
        $this->headers[] = 'OpenAI-Beta: assistants=v1';
        $url = OpenAiCoreUrl::threadsUrl() . '/' . $threadId . '/runs/' . $runId . '/submit_tool_outputs';
        $this->baseUrl($url);
    ...
    {
        $this->headers[] = 'OpenAI-Beta: assistants=v1';
        $url = OpenAiCoreUrl::threadsUrl() . '/' . $threadId . '/runs/' . $runId . '/cancel';
        $this->baseUrl($url);
    ...
    {
        $this->headers[] = 'OpenAI-Beta: assistants=v1';
        $url = OpenAiCoreUrl::threadsUrl() . '/runs';
        $this->baseUrl($url);
    ...
    {
        $this->headers[] = 'OpenAI-Beta: assistants=v1';
        $url = OpenAiCoreUrl::threadsUrl() . '/' . $threadId . '/runs/' . $runId . '/steps/' . $stepId;
        $this->baseUrl($url);
    ...
    {
        $this->headers[] = 'OpenAI-Beta: assistants=v1';
        $url = OpenAiCoreUrl::threadsUrl() . '/' . $threadId . '/runs/' . $runId . '/steps';
        if (count($query) > 0) {
            $url .= '?' . http_build_query($query);
        }
        $this->baseUrl($url);
    ...
    private function sendRequest(string $url, string $method, array $opts = [])
    {
        ...
        if (curl_errno($curl) > 0) {
            wLog("Open AI Curl Error",["errPayload" => curl_error($curl),"payloads" => $opts], logService()::LOG_OPEN_AI);

            $error_msg = curl_error($curl);

            session([sessionLab()::SESSION_OPEN_AI_ERROR => $error_msg]);

            throw new \Exception(localize("Open AI Failed communication") . curl_error($curl));
        }
...
//
//        if (! $response) {
//            wLog("Open AI Curl Error", $curl_info);
...
//        }
    ...
    {
        if ($this->customUrl != "") {
            $url = str_replace(OpenAiCoreUrl::ORIGIN, $this->customUrl, $url);
        }
    }
}