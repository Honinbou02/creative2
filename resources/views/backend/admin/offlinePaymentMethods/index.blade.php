...
@section('title')
    {{ localize('Offline Payment method') }} {{ getSetting('tab_separator') }} {{ getSetting('system_title') }}
    @endsection
@section("pagetitle", localize('Offline Payment method'))

@section('breadcrumb')
    @php
    $breadcrumbItems = [['href' => null, 'title' => 'Offline Payment method']]; @endphp
...
@section('pageTitleButtons')
    <div class="col-auto">
        <x-form.button type="button" id="addFrmOffCanvas" data-bs-toggle="offcanvas"
            data-bs-target="#addOfflinePaymentMethodFormSidebar">
            <i data-feather="plus"></i>{{ localize('Add OfflinePayment Method') }}
        </x-form.button>
    </div>
@endsection
...
@section('content')
    ...
    @include('backend.admin.offlinePaymentMethods.sidebar-add-method')
    <!-- Add Template Category -->
@endsection
@section('js')
    @include('backend.admin.offlinePaymentMethods.js')
@endsection
